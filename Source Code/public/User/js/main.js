...
function updateDeviceUI(devices) {
    ...
    if (devices.CBAS) {
        document.getElementById("CBAS-status").textContent = devices.CBAS.status === "BRIGHT" ? "Sáng" : "Tối" ?? "--";
        document.getElementById("CBAS-last").textContent = new Date().toLocaleTimeString();
    }
}
